...
local function bg_load()
  ...
  return { enabled = false, mode = 'file', path = '', index = 1 }
end
  ...
  window:perform_action(
    act.InputSelector {
      ...
      choices     = {
        ...
        { id = 'next',   label = '4  Next image  (folder mode only)'           },
        { id = 'prev',   label = '5  Previous image  (folder mode only)'       },
        { id = 'opacity',label = '6  Adjust opacity (edit config manually)'    },
      },
